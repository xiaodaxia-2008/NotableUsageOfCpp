...
public:
    BaseNode(std::string name) : m_name(std::move(name))
    {
        ...
        m_map = {{"one", 1.0}, {"two", 2.0}};
        m_unordered_map = {{"one", 1.0}, {"two", 2.0}};
        m_set.insert("1.0");
        m_set.insert("2.0");
    ...
    void serialize(Archive &ar)
    {
        ...
        ar(m_id, m_number, m_number1, m_number2);
        ar(m_variant);
    ...
    {
        fmt::format_to(ctx.out(), "{} at {} with {} children", GetName(),
                       fmt::ptr(this), GetChildrenCount());
